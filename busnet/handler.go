...
type MsgHandle struct {
	...
	TaskQueue      []chan busface.IRequest    //Worker负责取任务的消息队列
}
...
func NewMsgHandle() *MsgHandle {
	return &MsgHandle{
		...
		WorkerPoolSize: 20,
		//一个worker对应一个queue
		TaskQueue: make([]chan busface.IRequest, 20),
	}
}

//SendMsgToTaskQueue 将消息交给TaskQueue,由worker进行处理
func (mh *MsgHandle) SendMsgToTaskQueue(request busface.IRequest) {
...
func (mh *MsgHandle) StartWorkerPool() {
	...
	for i := 0; i < int(mh.WorkerPoolSize); i++ {
		...
		mh.TaskQueue[i] = make(chan busface.IRequest, 20)
